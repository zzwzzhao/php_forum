...
class User
{
	...
	//return if username is vaild format
	...
	public static function getById($user_id) {
		...
		$query = sprintf('SELECT USERNAME, PASSWORD, EMAIL_ADDR, IS_ACTIVE, ' .
		    'PERMISSION FROM %sUSER WHERE USER_ID = %d',DB_TBL_PREFIX, $user_id);
		$result = mysql_query($query, $GLOBALS['DB']);
		 if (!$result) echo "sql: ".mysql_error();
	...
	public static function getByUsername($username) {
		...
		$query = sprintf('SELECT USER_ID, PASSWORD, EMAIL_ADDR, IS_ACTIVE, '.
		    'PERMISSION FROM %sUSER WHERE USERNAME="%s"', DB_TBL_PREFIX,
	...
	public function save() {
		if ($this->uid) {
			$query = sprintf('UPDATE %sUSER SET USERNAME = "%s", ' .
				...
				$this->isActive, $this->permission, $this->userId);
