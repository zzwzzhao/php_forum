...
$sort = (isset($_GET['sort'])) ? $_GET['sort'] : 'LASTREPLY';
...
if ($forum_id) {
	...
	echo '<ul class="dropdown"><li><a href="view.php"> 回到板块列表 </a></li>';
	// display option to add new post if user is logged in
	if (isset($_SESSION['access'])) {
	    echo '<li><a href="add_post.php?fid=' . $forum_id . '"> 发布新帖 ' .
		'</a></li><li><a href=#>排序</a><ul><li><a href="view.php?fid=' . $forum_id . '&sort=MESSAGE_DATE">按发帖时间排序</a></li> ' .
		'<li><a href="view.php?fid=' . $forum_id .'&sort=LASTREPLY">按最后回帖时间排序</a></li></ul></li>';
	}
	echo '</ul>';
    }
}else {
...
if ($forum_id && $msg_id) {
    ...
    $display = 15; 
    $query = sprintf('SELECT UNIX_TIMESTAMP(LASTREPLY) AS LASTREPLY, ELITE, MESSAGE_ID, SUBJECT, USERNAME, ' .
	'UNIX_TIMESTAMP(MESSAGE_DATE) AS MESSAGE_DATE FROM  %sFORUM_MESSAGE M JOIN %sUSER U ON M.USER_ID = U.USER_ID '.
	'WHERE PARENT_MESSAGE_ID = 0 AND FORUM_ID = %d ORDER BY %s DESC', DB_TBL_PREFIX, DB_TBL_PREFIX, $forum_id, $sort);
    $result = mysql_query($query, $GLOBALS['DB']);
	...
	if($user->permission & User::MOVE_MESSAGE) {
	    echo '<th class="elite">加精</th>';
	}
	    echo '</tr></thead><tbody>';
	$count = 0;
	while (($row = mysql_fetch_assoc($result)) && ($count++ < $display)) {
	    $elite = ($row['ELITE'] == 1) ? '精华' : '';
	    echo '<tr><td class="subject"><a href="view.php?fid=' . $forum_id . '&mid=' .
		$row['MESSAGE_ID'] . '">' . htmlspecialchars($row['SUBJECT']). $elite . '</td>';
		...
		echo '<td class="delete"><a href="delete_message.php?fid=' . $forum_id . '&mid=' .
	     ...
	     if ($user->permission & User::DELETE_MESSAGE) {
		echo '<td class="elite"><a href="set_elite.php?fid=' . $forum_id . '&mid=' .
		    $row['MESSAGE_ID'] . '">加精</td>';
	    }
	    echo '</tr>';
	}
	echo '</tbody></table>';
